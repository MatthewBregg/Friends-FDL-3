...
// NOTE: THIS  CHIP IS CUT OFF ON THIS BUILD.
// --  - A3 - INA1
// --  - A2 - INA2
// --  - A4 - INB1
// --  - A5 - INB2
// PB3 - 11 - PWM A/B
// Chip A: (The bottom one, closer to the AVR) (Connec to pusher).
// --  - A0 - INB2
// --  - 13 - INB1
// --  - 5  - INA1
// --  - 8  - INA2
...
// Returns true IFF the limit switch is depressed.
...
void selftest() {
  ...
  if (digitalRead(trigSwitchB) && digitalRead(trigSwitchA)) {
...
void setup() {
  ...
  digitalWrite(A_INA1, HIGH);
  digitalWrite(A_INA2, HIGH);
  // We are in single channel mode, so set the B outputs to be low.
  digitalWrite(A_INB1, LOW);
  digitalWrite(A_INB2, LOW);
...
void loop() {
  ...
  if (currTrigState && prevTrigState) {
    ...
    unsigned long finished_d0 = 0;
    unsigned long finished_d1 = 0;
    //start flywheels
    OCR1A = 500; //go
    OCR1B = 500; //go
    ...
    if ((millis() - lastTriggerDown) >= failTime) {
      ...
      while ((goodTachCount < goodTachsToFire) && ((millis() - lastTriggerDown) <= failTime)) {
        ...
        if ((pulseLength0 <= (speedSetpoint + speedOffsetMargin)) && finished_d0 == 0) {
          finished_d0 = millis();
        }
        if ((pulseLength1 <= (speedSetpoint + speedOffsetMargin)) && finished_d1 == 0) {
          finished_d1 = millis();
        }
...
ISR(INT1_vect) {
  //pin 3 tach input handler - motor 0
...
ISR(INT0_vect) {
  //pin 2 tach input handler - motor 1
